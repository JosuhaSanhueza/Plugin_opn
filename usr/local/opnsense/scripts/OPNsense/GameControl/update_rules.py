...
def generate_unbound_rules():
    ...
    # 2. Generar zona RPZ (Response Policy Zone) nativa para Unbound
    rpz_lines = [
        "$TTL 30",
    ...
    for domain in blocked_domains:
        rpz_lines.append(f"{domain} CNAME .")
        rpz_lines.append(f"*.{domain} CNAME .")

    rpz_file = '/var/unbound/etc/gamecontrol.rpz'
    os.makedirs(os.path.dirname(rpz_file), exist_ok=True)
    with open(rpz_file, 'w') as f:
        f.write("\n".join(rpz_lines))

    # 3. Escribir archivo de configuración de tags en Unbound usando RPZ por etiqueta
    lines = [
    ...
    lines.append('rpz:')
    lines.append(f'    name: "gamecontrol_rpz"')
    lines.append(f'    zonefile: "{rpz_file}"')
    lines.append('    tags: "gaming_blocked"')
    lines.append('    rpz-action-override: cname-nobody')
