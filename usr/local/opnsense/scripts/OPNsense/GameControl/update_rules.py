...
def generate_unbound_rules():
    ...
    # Aplicar el etiquetado de Unbound para las IPs que NO están en la lista de permitidas
    lines = [
    ...
    # Asignar etiqueta "gaming_blocked" a las IPs que NO han sido habilitadas
    # Si una IP está en unblocked_ips, NO se le asigna la etiqueta y por lo tanto Unbound le permite navegar libremente
    hosts_elem = root.findall('.//OPNsense/GameControl/hosts/host')
    for host in hosts_elem:
        ip = host.find('ip')
        if ip is not None and ip.text and ip.text not in unblocked_ips:
            lines.append(f'    access-control-tag: {ip.text}/32 "gaming_blocked"')

    # En caso de que no haya un XML guardado explícito, aplicar a todo el rango excepto a las IPs permitidas (unblocked_ips)
    ...
    log_msg(f"IPs con bloqueo activo en Unbound: {145 - 101 + 1 - len(unblocked_ips)} (Permitidas: {len(unblocked_ips)})")
