...
def update_game_control():
    ...
    # 3. Configurar reglas Adblock con excepciones explícitas en AdGuard Home
    # En AdGuard Home:
    # - ||domain^$client=IP  -> Bloquea a esa IP.
    # - @@||domain^$client=IP -> EXCEPCIÓN: Permite explícitamente a esa IP.
    user_rules = ["! Auto-generated by OPNsense Escolarapp Game Manager"]
    
    # 1. Agregar bloqueo global de la lista a nivel de red
    for domain in domains:
        user_rules.append(f"||{domain}^")

    # 2. Agregar reglas de EXCEPCIÓN (@@) únicamente para las IPs que el profesor habilitó (Botón Verde)
