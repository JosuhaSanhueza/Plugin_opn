...
def update_game_control():
    log_msg("Sincronizando exenciones oficiales por cliente individual ($client=IP)... ")
    ...
    # Sintaxis Oficial Adblock / AdGuard Home: Una declaracion explicita por cada cliente IP individual
    user_rules = ["! Auto-generated by OPNsense Escolarapp Game Manager"]
    for ip_addr in sorted(list(unblocked_ips)):
        for domain in domains:
            user_rules.append("@@||" + domain + "^$client=" + ip_addr)
            user_rules.append("@@||*." + domain + "^$client=" + ip_addr)

    log_msg("Generando un total de " + str(len(user_rules)) + " lineas de exencion individuales en AdGuard Home")
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=5) as resp:
            rules_written = True
            log_msg("Excepciones individuales (" + str(len(user_rules)) + " lineas) sincronizadas via API.")
    except Exception:
        pass
